...
- **对话页**：消息列表（用户/助手气泡、节点状态条如「意图识别→数据查询」、错误条）+ 输入栏 + 快捷问题 chips。首轮 chips = 3 个通用问题 + 1 个「三年分数线走势」chip：有上下文时取用户最近浏览的分析结果里的第一所志愿学校（result 页写入本地），无上下文时发通用问题「查一所高中的三年分数线走势」——后端 Planner 检测到缺校名会走 Clarify 追问「哪所高中」（可点选热门校或输入简称）。（注：曾用「XX 中学」占位模板直发原文导致查无此校，2026-08-16 改为上下文驱动 + 缺槽追问；chips 文案必须是可直答或可追问的完整问题，勿写死示例校。）
- **初中语境记忆**：用户首次提供初中后写入 session slots，后续「我们学校」自动指代——个性化体验关键细节。
- **追问交互（HITL 产品形态）**：缺槽位出可点选项 chips（16 区、一模/二模）；校名追问的选项**按考生所在区动态取**（区内最新年份平行志愿线降序的头部校，ClarifyOptionsProvider 数据源），区未知时无选项、由用户自由输入；run_simulation 前出「确认开始模拟」按钮。
