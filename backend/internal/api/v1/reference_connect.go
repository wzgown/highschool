...
func (h *ReferenceServiceHandler) GetVolunteerRecommendations(
	ctx context.Context,
	req *connect.Request[highschoolv1.GetVolunteerRecommendationsRequest],
) (*connect.Response[highschoolv1.GetVolunteerRecommendationsResponse], error) {
	// 参数校验
	if req.Msg.DistrictId == 0 {
		return nil, connect.NewError(connect.CodeInvalidArgument, fmt.Errorf("district_id is required"))
	}
	if req.Msg.TotalScore <= 0 {
		return nil, connect.NewError(connect.CodeInvalidArgument, fmt.Errorf("total_score must be positive"))
	}
	if req.Msg.ExamType == highschoolv1.ExamType_EXAM_TYPE_UNSPECIFIED {
		return nil, connect.NewError(connect.CodeInvalidArgument, fmt.Errorf("exam_type is required"))
	}

	// 调用推荐服务
	resp, err := h.recommendationService.GetVolunteerRecommendations(ctx, req.Msg)
	if err != nil {
		logger.Error(ctx, "get volunteer recommendations failed", err)
		return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("获取志愿推荐失败: %w", err))
	}
