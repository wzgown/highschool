...
type SchoolRepository interface {
	...
	// GetSchoolsByCutoffScoreRanking 获取按分数线排名的学校列表（用于志愿生成）
...
func (r *schoolRepo) getSchoolsForUnifiedWithCache(ctx context.Context, districtID int32, year int) ([]*highschoolv1.SchoolForUnified, error) {
	...
	rows, err := r.db.Query(ctx, `
		...
		LEFT JOIN ref_admission_score_unified u
			ON u.school_id = s.id AND u.district_id = $1 AND u.year = $3
		WHERE s.is_active = true
		GROUP BY s.id, s.full_name, s.code, s.district_id
		ORDER BY
		  CASE WHEN MAX(u.min_score) IS NULL THEN 1 ELSE 0 END,
		  MAX(u.min_score) DESC NULLS LAST,
		  CASE WHEN s.district_id = $1 THEN 0 ELSE 1 END,
		  s.full_name
	`, districtID, year, year-1) // 使用当年的计划数据，前一年的分数线数据
	if err != nil {
...
func (r *schoolRepo) getSchoolsByCutoffScoreRankingWithCache(ctx context.Context, districtID int32, year int) ([]*SchoolRankingInfo, error) {
	...
	rows, err := r.db.Query(ctx, `
		SELECT id, full_name, district_id, min_score
		FROM (
			SELECT s.id, s.full_name, s.district_id,
				COALESCE(
					(SELECT q.min_score FROM ref_admission_score_quota_district q
					 WHERE q.school_id = s.id AND q.district_id = $1
					 ORDER BY q.year DESC LIMIT 1),
					(SELECT u.min_score FROM ref_admission_score_unified u
					 WHERE u.school_id = s.id AND u.district_id = $1
					 ORDER BY u.year DESC LIMIT 1)
				) as min_score
			FROM ref_school s
			INNER JOIN ref_quota_unified_allocation_district p
				ON p.school_id = s.id AND p.district_id = $1 AND p.year = $2 AND p.quota_count > 0
			WHERE s.is_active = true
		) ranked
	...
	logger.Debug(ctx, "getSchoolsByCutoffScoreRankingWithCache: query result",
		logger.Int("district_id", int(districtID)),
		logger.Int("schools_count", len(schools)),
	)
