...
type quotaCache struct {
	...
	unifiedQuotas            map[int32]int  // key: schoolID -> quota (deprecated, for backward compatibility)
	unifiedQuotasByDistrict  map[string]int // key: "schoolID_districtID" -> quota (new method)
...
func NewQuotaRepository() QuotaRepository {
	return &quotaRepo{
		...
		cache: &quotaCache{
			...
			unifiedQuotasByDistrict: make(map[string]int),
...
func (r *quotaRepo) PreloadCache(ctx context.Context, districtID int32, middleSchoolID int32, year int) {
	...
	if r.cache.year != year {
		...
		r.cache.unifiedQuotasByDistrict = make(map[string]int)
	...
	if err == nil {
		...
		for rows3.Next() {
			var schoolID, distID int32
			var quota int
			if err := rows3.Scan(&schoolID, &distID, &quota); err == nil {
				key := fmt.Sprintf("%d_%d", schoolID, distID)
				r.cache.unifiedQuotasByDistrict[key] = quota
			}
		}
	}
...
// GetUnifiedPlanByDistrict 获取统一招生计划数（按区）
func (r *quotaRepo) GetUnifiedPlanByDistrict(ctx context.Context, schoolID int32, districtID int32, year int) (int, error) {
	// 检查缓存
	key := fmt.Sprintf("%d_%d", schoolID, districtID)
	r.cache.mu.RLock()
	if r.cache.year == year {
		if quota, ok := r.cache.unifiedQuotasByDistrict[key]; ok {
			r.cache.mu.RUnlock()
			return quota, nil
		}
	}
	...
	`, schoolID, districtID, year).Scan(&quota)
	if err != nil {
		// 如果在新表中没有找到，回退到旧方法（从 ref_admission_plan_summary 获取总数）
		// 这种情况下假设学校在该区有招生（因为旧数据没有分区信息）
		logger.Debug(ctx, "unified plan by district not found in new table, falling back to total unified plan",
			logger.Int("school_id", int(schoolID)),
			logger.Int("district_id", int(districtID)),
			logger.Int("year", year),
			logger.ErrorField(err),
		)
		return r.GetUnifiedPlan(ctx, schoolID, year)
	}
