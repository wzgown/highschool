...
func TestStrategyVolunteerGenerator_GenerateUnifiedVolunteers(t *testing.T) {
	...
	t.Run("should generate volunteers for high rank (top 5%)", func(t *testing.T) {
		// 高排名考生：排名 500（前 5%）
		// 应该选择分数线高的学校
		rank := 500
		volunteers := generator.GenerateUnifiedVolunteers(ctx, rank, totalCandidates, districtID, rng)
		...
		// 验证志愿数量合理
		t.Logf("Generated %d volunteers for rank %d: %v", len(volunteers), rank, volunteers)
	})

	t.Run("should generate volunteers for medium rank (top 30%)", func(t *testing.T) {
		// 中等排名：排名 3000（前 30%）
		rank := 3000
		volunteers := generator.GenerateUnifiedVolunteers(ctx, rank, totalCandidates, districtID, rng)
	...
	t.Run("should generate volunteers for low rank (bottom 30%)", func(t *testing.T) {
		// 低排名考生：排名 7000（后 30%）
		rank := 7000
		volunteers := generator.GenerateUnifiedVolunteers(ctx, rank, totalCandidates, districtID, rng)
...
func TestStrategyVolunteerGenerator_GenerateQuotaDistrictVolunteer(t *testing.T) {
	...
	t.Run("should generate quota district volunteer", func(t *testing.T) {
		rank := 3000
		volunteer := generator.GenerateQuotaDistrictVolunteer(ctx, rank, totalCandidates, districtID, rng)
...
func TestStrategyVolunteerGenerator_GenerateQuotaSchoolVolunteers(t *testing.T) {
	...
	t.Run("should return nil if no eligibility", func(t *testing.T) {
		rank := 3000
		volunteers := generator.GenerateQuotaSchoolVolunteers(ctx, rank, totalCandidates, districtID, middleSchoolID, false, rng)
	...
	t.Run("should generate quota school volunteers if has eligibility", func(t *testing.T) {
		rank := 3000
		volunteers := generator.GenerateQuotaSchoolVolunteers(ctx, rank, totalCandidates, districtID, middleSchoolID, true, rng)
