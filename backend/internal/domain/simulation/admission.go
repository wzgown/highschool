...
func (r *UnifiedRule) Execute(ctx context.Context, candidates []*Candidate) []AdmissionResult {
	...
	for _, c := range candidates {
		...
		// 如果没有各科分数，使用总分 - 综合素质评价
		score750 := c.ChineseScore + c.MathScore + c.ForeignScore + c.IntegratedScore + c.EthicsScore + c.HistoryScore + c.PEScore
		if score750 == 0 && c.TotalScore > 0 {
			score750 = c.TotalScore - c.ComprehensiveQuality
		}

		// Debug: 记录真实考生的分数计算
		if c.IsRealCandidate {
			logger.Info(ctx, "real candidate score750 calculation",
				logger.Float64("totalScore", c.TotalScore),
				logger.Float64("comprehensiveQuality", c.ComprehensiveQuality),
				logger.Float64("score750", score750),
				logger.Float64("chinese", c.ChineseScore),
				logger.Float64("math", c.MathScore),
			)
		}
	...
	for districtID, districtCands := range districtCandidates {
		...
		for rank, cs := range districtCands {
			...
			for _, schoolID := range c.UnifiedSchoolIDs {
				...
				// 获取该学校在该区的统一招生计划数（使用新方法）
				quota, err := r.quotaRepo.GetUnifiedPlanByDistrict(ctx, schoolID, districtID, r.year)
				if err != nil || quota <= 0 {
					quota = 100 // 默认值
				}
...
func (s *Simulator) Run(ctx context.Context, candidates []*Candidate) map[int32]*AdmissionResult {
	...
	for _, rule := range s.rules {
		...
		admittedThisBatch := notAdmitted - newNotAdmitted
		logger.Info(ctx, "batch admission stats",
			logger.String("batch", rule.Name()),
			logger.Int("admitted", admittedThisBatch),
			logger.Int("remaining", newNotAdmitted),
		)
		notAdmitted = newNotAdmitted
	}
