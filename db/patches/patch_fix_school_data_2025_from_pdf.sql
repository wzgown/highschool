...
SET code = '142003',
    updated_at = NOW()
WHERE id = 187 AND code = '144011';

-- 2. 上海大学附属嘉定高级中学：DB代码143004 → PDF代码144011
UPDATE ref_school
SET code = '144011',
    updated_at = NOW()
WHERE id = 297 AND code = '143004';

-- 3. 上海宋庆龄学校：DB代码185000 → PDF代码185008
UPDATE ref_school
SET code = '185008',
    updated_at = NOW()
WHERE id = 280 AND code = '185000';
...
INSERT INTO ref_school (
    ...
    boarding_type_id, has_international_course, is_active, created_at, updated_at
)
SELECT
    '094004',
    '上海市友谊中学',
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM ref_school WHERE code = '094004'
);
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM ref_school WHERE code = '143002'
);
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM ref_school WHERE code = '143003'
);
...
SET is_active = true,
    ...
    updated_at = NOW()
WHERE code = '155057' AND is_active = false;
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM ref_school WHERE code = '163000'
);
...
BEGIN
    SELECT COUNT(*) INTO v_total_count FROM ref_school ;
    SELECT COUNT(*) INTO v_active_count FROM ref_school  AND is_active = true;
    SELECT COUNT(*) INTO v_inactive_count FROM ref_school  AND is_active = false;
