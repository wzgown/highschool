...
INSERT INTO ref_admission_plan_summary (year, school_id, school_code, school_name, district_id, school_type_id, is_municipal, autonomous_count, autonomous_sports_count, autonomous_arts_count, quota_district_count, quota_school_count, unified_count, total_plan_count, created_at, updated_at)
SELECT
    2025 as year,
    s.id as school_id,
...
AND NOT EXISTS (
    ...
    WHERE p.school_id = s.id AND p.year = 2025
);
...
SELECT 'ref_school总数(2025)' as 检查项, COUNT(*)::text as 结果 FROM ref_school 
UNION ALL
SELECT '重复学校名称', COUNT(*)::text FROM (
    SELECT full_name FROM ref_school  GROUP BY full_name HAVING COUNT(*) > 1
) t
UNION ALL
...
WHERE p.year = 2025 AND NOT EXISTS (SELECT 1 FROM ref_school s WHERE s.id = p.school_id)
UNION ALL
...
WHERE p.year = 2025 AND p.school_name != s.full_name
UNION ALL
